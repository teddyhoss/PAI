...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # In produzione, specifica i domini esatti invece di "*"
    allow_credentials=True,
    allow_methods=["*"],  # Puoi specificare i metodi esatti: ["GET", "POST", "PUT", "DELETE"]
    allow_headers=["*"],  # Puoi specificare gli headers esatti se necessario
)
...
def get_stats(db: Session = Depends(get_db)):
    try:
        ...
        # Conteggio per urgenza alta usando l'operatore ->> per JSON
        high_urgency_count = db.query(models.Issue)\
            .filter(models.Issue.classification['urgency'].astext == 'high')\
            .count()
            
        # Distribuzione per categoria
        categories_query = db.query(
            models.Issue.classification['category'].astext.label('category'),
            func.count('*').label('count')
        ).group_by(models.Issue.classification['category'].astext).all()
        
        categories_distribution = {cat: count for cat, count in categories_query if cat}
        ...
        zones_distribution = {zone: count for zone, count in zones_query if zone}
