    ...
    try {
      ...
      const contextMessage = `
                ...
                ${statsData.recent_issues
                  .map(
                    (issue) =>
                      `- ${issue.text} (${issue.category}, ${issue.urgency})`,
                  )
                  .join("\n")}
            `;
          ...
          messages: [
            {
              ...
              content: `Sei un assistente della PA italiana che aiuta a interpretare le statistiche delle segnalazioni.
                            ...
                            Contesto attuale del sistema:
                            - Totale segnalazioni: ${statsData.total}
                            - Segnalazioni ad alta urgenza: ${
                              statsData.high_urgency_count
                            }
                            - Categoria più frequente: ${statsData.top_category}
                            - Zona più attiva: ${statsData.top_zone}
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "Mi dispiace, si è verificato un errore nella comunicazione.",
        },
      ]);
    } finally {
      setIsLoading(false);
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-gray-100 rounded-lg p-3">Sto pensando...</div>
          </div>
        )}
      </div>
